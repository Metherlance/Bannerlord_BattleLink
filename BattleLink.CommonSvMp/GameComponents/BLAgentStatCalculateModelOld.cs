...
public class BLAgentStatCalculateModelOld : MultiplayerAgentStatCalculateModel
{

    public BLAgentStatCalculateModelOld() : base()
    {
    ...
    private void UpdateHumanAgentStats(Agent agent, AgentDrivenProperties agentDrivenProperties)
    {
        ...
        float totalWeightWeapon = equipment.GetTotalWeightOfWeapons() * (float)(1.0 + (perkHandler != null ? (double)perkHandler.GetEncumbrance(true) : 0.0));
        ...
        if (wieldedItemIndex1 != EquipmentIndex.None)
        {
            ...
            if (weaponComponent != null)
            {
                ...
                float itemWieldedWeight1 = (weaponComponent.GetItemType() == ItemObject.ItemTypeEnum.Bow ? 4f : 1.5f) * itemObject.Weight * MathF.Sqrt(realWeaponLength) * (float)(1.0 + (perkHandler != null ? (double)perkHandler.GetEncumbrance(false) : 0.0));
                totalWeightWeapon += itemWieldedWeight1;
            }
        }
        if (wieldedItemIndex2 != EquipmentIndex.None)
        {
            float itemWieldedWeight2 = 1.5f * equipment[wieldedItemIndex2].Item.Weight * (float)(1.0 + (perkHandler != null ? (double)perkHandler.GetEncumbrance(false) : 0.0));
            totalWeightWeapon += itemWieldedWeight2;
        }
        agentDrivenProperties.WeaponsEncumbrance = totalWeightWeapon;
        ...
        agentDrivenProperties.MaxSpeedMultiplier = (float)(1.04999995231628 * ((double)movementSpeedMultiplier * (100.0 / (100.0 + (double)totalWeightWeapon))));

        //new (sp)
        int weightBody = agent.Monster.Weight;
        float armorEncumbrance = this.GetEffectiveArmorEncumbrance(agent, agent.SpawnEquipment);
        agentDrivenProperties.ArmorEncumbrance = armorEncumbrance;
        float totalWeight = armorEncumbrance + totalWeightWeapon;
        int effectiveSkillAthletics = this.GetEffectiveSkill(agent, DefaultSkills.Athletics);
        float maxValue = (float)(0.699999988079071 * (1.0 + 3.0 * 0.1f));// 0.1f (double)DefaultSkillEffects.AthleticsSpeedFactor.PrimaryBonus)
        // (double)DefaultSkillEffects.AthleticsWeightFactor.GetPrimaryValue(effectiveSkill1)
        float num4 = MBMath.ClampFloat((float)(0.699999988079071 + ((double)maxValue - 0.699999988079071) / 300.0 * (double)effectiveSkillAthletics) - MathF.Max((float)(0.200000002980232 * (1.0 - (.1f*effectiveSkillAthletics) * 0.00999999977648258)), 0.0f) * totalWeight / (float)weightBody, 0.0f, maxValue);
        agentDrivenProperties.MaxSpeedMultiplier = this.GetEnvironmentSpeedFactor(agent) * num4;


        int skillRidingValue = character.GetSkillValue(DefaultSkills.Riding);
        bool flag1 = false;
        bool flag2 = false;
        if (weaponComponentData != null)
        {
            ...
            if (weapon.IsRangedWeapon)
            {
                ...
                {
                    float num6 = MathF.Max(0.0f, (float)((1.0 - (double)effectiveSkillForWeapon / 500.0) * (1.0 - (double)skillRidingValue / 1800.0)));
        ...
        agentDrivenProperties.AttributeRiding = (float)skillRidingValue * num7;
